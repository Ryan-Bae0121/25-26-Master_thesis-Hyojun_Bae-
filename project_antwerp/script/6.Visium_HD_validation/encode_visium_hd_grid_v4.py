#!/usr/bin/env python3
"""
encode_visium_hd_grid_v4.py
============================
형님 방식으로 타일 크기 수정:
  um/px_hires = microns_per_pixel / tissue_hires_scalef
  tile_size   = round(target_um / um_per_px_hires)
  → 68um / 2.974 = 22.9px ≈ 23px

Usage:
    python encode_visium_hd_grid_v4.py \
...
def calc_tile_size(scalefactors_path, target_um=68.0):
    """
    형님 방식:
      um/px_fullres   = microns_per_pixel
      um/px_hires     = microns_per_pixel / tissue_hires_scalef
      tile_size (px)  = round(target_um / um_per_px_hires)
    """
    with open(scalefactors_path) as f:
        sf = json.load(f)
    microns_per_pixel    = sf['microns_per_pixel']       # um/px_fullres
    tissue_hires_scalef  = sf['tissue_hires_scalef']     # px_hires/px_fullres
    ...
    print(f'  tissue_hires_scalef: {tissue_hires_scalef:.8f} px_hires/px_fullres')
...
if __name__ == '__main__':
    ...
    # ★ 유전자 순서를 train gene_list와 동일하게 정렬
    ...
    print(f'    train gene_list 첫 3개: {hvg_genes[:3]}')
    ...
    # ── [5] 유효 타일 목록 생성
    print(f'\n[5] 유효 타일 목록 생성 (bin bounding box)')
    ...
    print(f'\n[7] 임베딩 추출 + bin aggregation')
    print(f'    방식: bin raw count 합산 → tile별 normalize_total + log1p')
