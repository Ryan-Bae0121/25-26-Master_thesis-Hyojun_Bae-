#!/usr/bin/env python3
"""
encode_visium_hd_grid_v2.py
============================
Visium HD 그리드 타일링 + 임베딩 추출 + bin aggregation

수정사항:
- 각 bin을 먼저 normalize_total + log1p (HNSCC spot과 동일한 스케일)
- 타일 내 bin들의 평균으로 aggregation
- 유전자 순서를 train_exprs와 동일하게 저장

Usage:
    python encode_visium_hd_grid_v2.py \
...
if __name__ == '__main__':
    ...
    # ── [4] count matrix 로드 + normalize (HNSCC spot과 동일한 방식)
    print(f'\n[4] count matrix 로드 + normalize')
    adata = sc.read_10x_h5(args.count_h5)
    adata.var_names_make_unique()

    # HVG 필터링 (train_exprs와 동일한 유전자)
    with open(args.gene_list) as f:
        hvg_genes = [line.strip() for line in f.readlines()]
    adata = adata[:, adata.var_names.isin(hvg_genes)].copy()

    # 유전자 순서를 train gene_list와 동일하게 맞추기
    gene_order = {g: i for i, g in enumerate(hvg_genes)}
    adata = adata[:, sorted(adata.var_names, key=lambda g: gene_order.get(g, 99999))].copy()

    print(f'    shape: {adata.shape}')
    print(f'    정규화 전: mean={adata.X.mean():.4f}')

    # ★ 핵심 수정: 각 bin을 먼저 normalize_total + log1p
    # → HNSCC spot 1개와 동일한 스케일
    sc.pp.normalize_total(adata)
    sc.pp.log1p(adata)

    X = adata.X.toarray() if hasattr(adata.X, 'toarray') else np.array(adata.X)
    gene_names = np.array(list(adata.var_names))
    print(f'    정규화 후: mean={X.mean():.4f}, std={X.std():.4f}')
    ...
    print(f'\n[8] 임베딩 추출 + bin aggregation (bin별 normalize 후 평균)')
    ...
    for row_start, col_start in tqdm(valid_tiles, desc='처리 중'):
        ...
        # bin aggregation: normalize된 bin들의 평균
        in_tile = (
            (bin_col >= col_start) & (bin_col < col_start + tile_size) &
            (bin_row >= row_start) & (bin_row < row_start + tile_size)
        )
        ...
        if len(tile_bin_idx) > 0:
            # ★ sum 대신 mean → HNSCC spot 1개와 동일한 스케일
            tile_expr = X[tile_bin_idx].mean(axis=0)
        else:
    ...
    print(f'    tile_exprs mean: {all_exprs.mean():.4f}, std: {all_exprs.std():.4f}')
    ...
    np.save(os.path.join(args.output_dir, 'tile_exprs.npy'),     all_exprs)
