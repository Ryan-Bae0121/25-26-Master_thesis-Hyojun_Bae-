#!/usr/bin/env python3
"""
encode_visium_hd_grid_v3.py
============================
Visium HD 그리드 타일링 + 임베딩 추출 + bin aggregation

형님 지시사항 반영:
- bin raw count 합산 후 tile별 normalize_total + log1p
- 유전자 순서를 train gene_list와 동일하게 저장

Usage:
    python encode_visium_hd_grid_v3.py \
...
if __name__ == '__main__':
    ...
    # ── [4] count matrix 로드 (raw count, normalize 전)
    ...
    # ── [8] 임베딩 추출 + bin aggregation (raw count 합산)
    print(f'\n[8] 임베딩 추출 + bin aggregation')
    print(f'    방식: bin raw count 합산 → 나중에 tile별 normalize_total + log1p')
    ...
    # ★ tile별 normalize_total + log1p (형님 지시)
