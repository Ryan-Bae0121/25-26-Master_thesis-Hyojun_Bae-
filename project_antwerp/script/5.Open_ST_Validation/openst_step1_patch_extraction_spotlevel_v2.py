...
# ── Normalize 순서 중요 ────────────────────────────────────────────────────
# Visium pipeline: 전체 gene normalize_total(1e4) → log1p → HVG subset
# → HVG subset 후 expm1 sum ≈ 6959 (HVG가 전체 발현의 ~70%)
# Open-ST도 동일하게: 전체 gene 기준 normalize 후 HVG subset
# (HVG subset 후 normalize하면 target=10000이 되어 scale 불일치 발생)
print("  Normalizing (전체 gene 기준 → HVG subset, Visium pipeline과 동일) ...")
row_sums_all = raw.sum(axis=1, keepdims=True)   # 전체 gene 합
row_sums_all[row_sums_all == 0] = 1
raw_norm_all = raw / row_sums_all * 1e4          # normalize_total(1e4), 전체 gene 기준
raw_norm_all = np.log1p(raw_norm_all)            # log1p
raw_hvg = raw_norm_all[:, hvg_col_idx]           # HVG subset → sum ≈ 7000
print(f"  First cell expm1 sum after norm+subset: {np.expm1(raw_hvg[0]).sum():.0f} (Visium: ~6959)")
...
raw_tissue    = raw[in_tissue]         # 전체 gene raw counts (aggregation용)
...
for out_i, src_i in enumerate(valid_idx):
    ...
    # raw counts 합산 (전체 gene 기준 normalized 값 합산)
    # raw_hvg는 이미 전체 gene 기준 normalize_total+log1p 후 HVG subset된 값
    # → 합산하면 안 됨. raw counts를 합산 후 다시 normalize해야 함
    fov_raw = raw_tissue[fov_cell_idx]   # (n_fov_cells, n_all_genes) raw counts
    fov_sum = fov_raw.sum(axis=0)   # 전체 gene 합산
    # Visium과 동일하게: 전체 gene 기준 normalize → log1p → HVG subset
    fov_total = fov_sum.sum()
    if fov_total > 0:
        fov_norm = fov_sum / fov_total * 1e4
    else:
        fov_norm = fov_sum
    fov_log = np.log1p(fov_norm)
    expr_agg[out_i] = fov_log[hvg_col_idx]   # HVG subset
...
print(f"  expm1 sum mean: {np.expm1(expr_agg).sum(axis=1).mean():.0f} (Visium: ~6959)")
