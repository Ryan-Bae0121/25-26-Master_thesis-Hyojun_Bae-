#!/usr/bin/env python3
"""
Top-k Experiment - Find optimal top_k for Var ratio ≈ 1.0
Reuses saved similarity matrix from tau experiment
python3 predex_topk_experiment.py
```

**예상 시간: ~5분** (similarity matrix 재사용!)

**예상 결과:**
```
top_k=100   → Var ratio 4.44 (현재)
top_k=500   → Var ratio ~2.0?
top_k=1000  → Var ratio ~1.5?
top_k=5000  → Var ratio ~1.0? ⭐
top_k=10000 → Var ratio ~0.8?
"""
...
def main():
    ...
    # ============================================================
    FOLD = 1
    TAU = 0.1  # Best tau from previous experiment
    TOPK_LIST = [10, 50, 100, 500, 1000, 5000, 10000]
    VAL_SAMPLE = 1000
    ...
    OUT_DIR = Path("/project_antwerp/hbae/Loki_output/topk_experiment/fold_01")
    DEVICE = "cuda"
    ...
    # Summary
    print("\n" + "="*80)
    print("TOP-K EXPERIMENT SUMMARY (tau=0.1)")
    print("="*80)
    print(f"{'top_k':<10} {'Spot Corr':<20} {'Gene Corr':<20} {'Var Ratio':<15} {'Var→1.0?'}")
    print("-"*75)
    for r in all_results:
        dist_from_1 = abs(r['var_ratio_mean'] - 1.0)
        best = "⭐" if dist_from_1 < 0.5 else ""
        print(f"{r['top_k']:<10} "
              f"{r['spot_corr_mean']:.4f}±{r['spot_corr_std']:.4f}  "
              f"{r['gene_corr_mean']:.4f}±{r['gene_corr_std']:.4f}  "
              f"{r['var_ratio_mean']:<15.4f} {best}")
    print("="*80)
    ...
    print(f"\n✅ Best Spot Corr: top_k={best_spot['top_k']} ({best_spot['spot_corr_mean']:.4f})")
    print(f"✅ Var ratio closest to 1.0: top_k={best_var['top_k']} ({best_var['var_ratio_mean']:.4f})")
    print("="*80)

    # Save
    pd.DataFrame(all_results).to_csv(OUT_DIR / 'topk_experiment_results.csv', index=False)
    with open(OUT_DIR / 'topk_experiment_results.json', 'w') as f:
