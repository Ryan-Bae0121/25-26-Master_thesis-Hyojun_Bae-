...
def loki_predex_weighted_average(test_img_emb, train_text_embs, train_exprs, 
                                  temperature=0.07, top_k=None):
    """
    Loki PredEx: 가중 평균으로 유전자 발현 예측
    
    Args:
        test_img_emb: (D,) - 테스트 이미지 임베딩
        train_text_embs: (N, D) - 훈련 텍스트 임베딩
        train_exprs: (N, G) - 훈련 발현량
        temperature: Similarity scaling (논문 기본값: 0.07)
        top_k: 상위 k개만 사용 (None이면 전체)
    
    Returns:
        predicted_expr: (G,) - 예측된 발현량
    """
    ...
    # 4. Softmax로 가중치 계산
    weights = F.softmax(similarities_k, dim=0)  # (N,) or (k,)
    
    # 5. 가중 평균
    predicted_expr = (weights[:, None] * train_exprs_k).sum(dim=0)  # (G,)
...
def main(args):
    ...
    print("[7] Running Loki PredEx (weighted average)...")
...
if __name__ == "__main__":
    ...
    p.add_argument("--temperature", type=float, default=0.07, 
                   help="Temperature for similarity scaling (논문 기본값: 0.07)")
