...
def main(args):
    ...
    # (A) Spot-wise correlation (with per-spot normalization)
    spot_corrs = []
    spot_corrs_spearman = []
    for i in range(len(predictions)):
        try:
            # Per-spot z-score normalization
            pred_i = predictions[i]
            gt_i = ground_truths[i]
            
            # Z-score normalization
            pred_norm = (pred_i - pred_i.mean()) / (pred_i.std() + 1e-8)
            gt_norm = (gt_i - gt_i.mean()) / (gt_i.std() + 1e-8)
            
            # Pearson on normalized
            corr, _ = pearsonr(pred_norm, gt_norm)
            spot_corrs.append(corr if np.isfinite(corr) else 0.0)
            
            # Spearman (rank-based, scale-invariant)
            from scipy.stats import spearmanr
            corr_s, _ = spearmanr(pred_i, gt_i)
            spot_corrs_spearman.append(corr_s if np.isfinite(corr_s) else 0.0)
        except:
            spot_corrs.append(0.0)
            spot_corrs_spearman.append(0.0)
    
    spot_corrs = np.array(spot_corrs)
    spot_corrs_spearman = np.array(spot_corrs_spearman)
    
    # (B) Gene-wise correlation (with per-gene normalization)
    gene_corrs = []
    gene_corrs_spearman = []
    for g in range(predictions.shape[1]):
        try:
            ...
            # Z-score normalization
            pred_norm = (pred_g - pred_g.mean()) / (pred_g.std() + 1e-8)
            gt_norm = (gt_g - gt_g.mean()) / (gt_g.std() + 1e-8)
            ...
            # Spearman
            from scipy.stats import spearmanr
            corr_s, _ = spearmanr(pred_g, gt_g)
            gene_corrs_spearman.append(corr_s if np.isfinite(corr_s) else 0.0)
        except:
            gene_corrs.append(0.0)
            gene_corrs_spearman.append(0.0)
    
    gene_corrs = np.array(gene_corrs)
    gene_corrs_spearman = np.array(gene_corrs_spearman)
    ...
    results = {
        ...
        "spot_wise_corr_pearson_normalized": {
        ...
        "spot_wise_corr_spearman": {
            "mean": float(spot_corrs_spearman.mean()),
            "median": float(np.median(spot_corrs_spearman)),
            "std": float(spot_corrs_spearman.std()),
            "min": float(spot_corrs_spearman.min()),
            "max": float(spot_corrs_spearman.max()),
        },
        "gene_wise_corr_pearson_normalized": {
        ...
        "gene_wise_corr_spearman": {
            "mean": float(gene_corrs_spearman.mean()),
            "median": float(np.median(gene_corrs_spearman)),
            "std": float(gene_corrs_spearman.std()),
            "min": float(gene_corrs_spearman.min()),
            "max": float(gene_corrs_spearman.max()),
        },
    }
    ...
    print()
    print("Spot-wise Pearson (normalized):")
    print(f"  Mean   : {results['spot_wise_corr_pearson_normalized']['mean']:.4f}")
    print(f"  Median : {results['spot_wise_corr_pearson_normalized']['median']:.4f}")
    print(f"  Std    : {results['spot_wise_corr_pearson_normalized']['std']:.4f}")
    print()
    print("Spot-wise Spearman (rank):")
    print(f"  Mean   : {results['spot_wise_corr_spearman']['mean']:.4f}")
    print(f"  Median : {results['spot_wise_corr_spearman']['median']:.4f}")
    print(f"  Std    : {results['spot_wise_corr_spearman']['std']:.4f}")
    print()
    print("Gene-wise Pearson (normalized):")
    print(f"  Mean   : {results['gene_wise_corr_pearson_normalized']['mean']:.4f}")
    print(f"  Median : {results['gene_wise_corr_pearson_normalized']['median']:.4f}")
    print(f"  Std    : {results['gene_wise_corr_pearson_normalized']['std']:.4f}")
    print()
    print("Gene-wise Spearman (rank):")
    print(f"  Mean   : {results['gene_wise_corr_spearman']['mean']:.4f}")
    print(f"  Median : {results['gene_wise_corr_spearman']['median']:.4f}")
    print(f"  Std    : {results['gene_wise_corr_spearman']['std']:.4f}")
    print()
