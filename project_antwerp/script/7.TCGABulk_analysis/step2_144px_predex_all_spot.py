"""
Step 2: 144px Embedding으로 PredEx + Sliding Window + Bulk PCC
기존 코드에서 변경사항:
  1. TCGA_EMB_DIR → TCGA_embeddings_144px
  2. FINETUNE_EMB → 0317_epoch10_finetune_embedding_new
  3. GENE_LIST    → 0317_hvg_2000_list.txt
  4. OUTPUT_DIR   → finetuned_ensemble_144px
  5. slide_id 매칭: wsi_file_name + ".svs" (fold_03 기준)
"""
...
OUTPUT_DIR    = "/project_antwerp/hbae/Loki_output/TCGA_bulk_prediction/finetuned_ensemble_144px"

WINDOW_SIZE = 10
STRIDE      = 5
MIN_TILES   = 50
FOLDS       = ["fold_03"]  # 현재 완료된 fold만
...
# slide_id: wsi_file_name + ".svs" (144px 폴더명 기준)
...
for fold in FOLDS:
    ...
    for sid in tqdm(matched_sids, desc=f"  {fold}", leave=False):
        ...
        slide_pred = slide_pred_2000[common_in_hvg_idx]  # (common_genes,)
