...
사용법:
  ...
  python3 build_combined_obs_and_expression.py \
    ...
    python3 build_combined_obs_and_expression.py \
    --meta-csv /project_antwerp/hbae/data/0317_training_data_excluding_GSE220978_and_19h1257/training_data_excluding_GSE220978_and_19h1257/0228_HVG_Finetune_meta_excluding_GSE220978_and_19h1257__ordered_like_combined_obs.csv \
    --gene-list /project_antwerp/hbae/data/0317_training_data_excluding_GSE220978_and_19h1257/training_data_excluding_GSE220978_and_19h1257/HVG_2000_genes.txt \
    --merged-h5ad /project_antwerp/hbae/data/0317_training_data_excluding_GSE220978_and_19h1257/merged_all_st_norm.h5ad \
    --out-dir /project_antwerp/hbae/data/0317_training_data_excluding_GSE220978_and_19h1257
...
def main():
    ...
    if args.merged_h5ad and Path(args.merged_h5ad).exists():
        ...
        # Build (n_obs, n_genes) in meta order; 벡터화로 속도 개선
        row_idx_arr = np.array(row_indices, dtype=np.int64)
        valid_row = row_idx_arr >= 0
        var_to_col = {g: j for j, g in enumerate[str](var_in)}
        expr_matrix = np.zeros((n_obs, n_genes), dtype=np.float32)
        for j in range(n_genes):
            c = var_to_col.get(genes_want[j], -1)
            if c >= 0:
                expr_matrix[valid_row, j] = X_full[row_idx_arr[valid_row], c]
