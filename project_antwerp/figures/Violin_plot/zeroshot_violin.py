#!/usr/bin/env python3
"""
Zero-shot violin plot - violin_tido.py 와 완전히 동일한 스타일
"""
...
# ── 데이터 로드
emb_dir = Path('/project_antwerp/hbae/Loki_output/0228_embeddings_zeroshot')
merged = defaultdict(list)
found_folds = []

for fold_dir in sorted(emb_dir.glob('fold_*')):
    psd = fold_dir / 'per_sample_dist'
    if not psd.exists(): continue
    found_folds.append(fold_dir.name)
    for npy_file in sorted(psd.glob('*_gene_pcc.npy')):
        sid = npy_file.stem.replace('_gene_pcc', '')
...
title = (f"Gene-wise PCC per Sample  "
         f"(Top 300 genes · Zero-shot · {len(found_folds)} folds · {n} samples)")
draw_violin(ax, sids, data, title)

plt.tight_layout()
plt.subplots_adjust(right=0.78)
out = Path('/project_antwerp/hbae/script/0208_start/zeroshot_violin.png')
