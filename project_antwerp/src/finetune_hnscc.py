...
OPENCLIP_ROOT = Path(os.getenv("OPENCLIP_ROOT", "/data/hbae/open_clip"))  # open_clip 레포 위치
PRETRAINED = Path(os.getenv("PRETRAINED", "/data/hbae/checkpoint.pt"))  # omiclip ckpt
CSV_PATH = Path(os.getenv("CSV_PATH", "/data/hbae/Loki_Finetuning/HEG_finetune_meta.csv"))

# 모델 설정 (환경 변수로 오버라이드 가능)
model_name = os.getenv("MODEL_NAME", "coca_ViT-L-14")
name = os.getenv("EXPERIMENT_NAME", "finetune_HEG_hnscc")
